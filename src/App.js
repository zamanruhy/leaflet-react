...
export default function App({ zoom = 13, center = {}, markers = [] }) {
  const [selected, setSelected] = useState([])

  // const types = markersOld.reduce(
  ...
  const filteredMarkers =
    selected.length === 0
      ? markers
      : markers.filter((marker) => selected.includes(marker.id))
  ...
  return (
    <>
      ...
      <div className='flex gap-3'>
        ...
        {markers.map((marker) => (
          <button
            type='button'
            key={marker.id}
            className={selected.includes(marker.id) && 'font-bold'}
            onClick={(e) => onClick(e, marker.id)}
          >
            <img src={marker.menuIcon} aria-hidden='true' alt='' />
            {marker.name}
          </button>
        ))}
      </div>
      <MapContainer
        style={{ height: '80vh' }}
        center={strToPos(center.position)}
        zoom={15}
      >
        <TileLayer
        ...
        {/* Center marker */}
        <Marker
          position={strToPos(center.position)}
          icon={
            new L.Icon({
              iconUrl: center.icon,
              iconSize: [40, 40],
        ...
        {filteredMarkers.map((marker) =>
          marker.points.map((point) => (
            <Marker
              key={`${marker.id}${point.id}`}
              position={strToPos(point.position)}
              icon={
                new L.Icon({
                  iconUrl: marker.mapIcon,
                  // iconAnchor: null,
                  popupAnchor: [0, -20],
                  iconSize: [40, 40],
                })
        ...
        <Marker
          position={[
            strToPos(center.position)[0],
            strToPos(center.position)[1] + 0.002,
          ]}
          icon={
            new L.Icon({
        ...
        <Marker
          position={[
            strToPos(center.position)[0],
            strToPos(center.position)[1] + 0.0045,
          ]}
          icon={
            new L.Icon({
        ...
        <Marker
          position={[
            strToPos(center.position)[0],
            strToPos(center.position)[1] + 0.0075,
          ]}
          icon={
            new L.Icon({
